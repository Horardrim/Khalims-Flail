...
@Component({
  selector: 'app-remove-duplicates-i',
  standalone: true,
  imports: [CommonModule, FormsModule, HighchartsChartComponent],
  templateUrl: './remove-duplicates-i.component.html',
  styleUrls: ['./remove-duplicates-i.component.css']
})
export class RemoveDuplicatesIComponent {
  Highcharts = Highcharts;
  inputValue = '1,1,2,2,3,3,3,4';
  ...
  get resultLength(): number {
    ...
    return this.currentSnapshot.slow + 1;
  }
  ...
  private buildRemoveDuplicateSteps(input: number[]): RemoveDuplicateStep[] {
    ...
    pushStep('初始化：slow=0，第一个元素默认保留', 0);
    ...
    for (let fast = 1; fast < arr.length; fast++) {
      if (arr[fast] !== arr[slow]) {
        ...
        pushStep(`nums[fast]=${arr[fast]} != nums[slow-1]=${arr[slow-1]}，保留该元素，slow++`, fast);
      } else {
        removed.push(arr[fast]);
        pushStep(`nums[fast]=${arr[fast]} == nums[slow]=${arr[slow]}，重复元素，跳过`, fast, true);
      }
    }

    pushStep(`去重完成！新数组长度为 ${slow + 1}，共移除 ${removed.length} 个重复项`, arr.length - 1, true);
    return steps;
  }
  ...
  private updateChart(): void {
    ...
    const seriesData = snapshot.array.map((value, index) => {
      ...
      if (index === snapshot.slow && index !== snapshot.fast) {
        label = 'slow';
      }
    ...
    this.chartOptions = {
      ...this.chartOptions,
      title: {
        text: `删除重复项 I 可视化 (slow=${snapshot.slow}, fast=${snapshot.fast})`
      },
      xAxis: {
